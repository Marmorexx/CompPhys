...
# Create a random n-dimensional Matrix with Spin states -1 and 1
def createrandom(n):
    Matrix = np.round(np.random.rand(n,n),0).astype(int)
    for i in range(n):
        for j in range(n):
            if Matrix[i][j] < 0.5: Matrix[i][j] = -1
    return Matrix


###############################################################################
### Create a random Matrix s_a
s_a = createrandom(6)
B = 2
J = 1
###############################################################################
